...
VALUES (NULL,'$uID','$d_source_confirmed_location', '$d_source_state', '$d_source_district', '$d_source_panchayath','$d_source_ward_no', '$d_source_in_date','$d_source_up_date')";
	...
	if($insertresult) 
	{
		$msg      = "Source successfully added.";
        $msg_type = "Success";
		echo "<script>location='sourceView.php?msg=" . urlencode($msg) . "&msg_type=" . urlencode($msg_type) . "'</script>";
        exit();
	} 
	else 
	{
...
if(isset($_POST["btncontactadd"]))
{
	...
	//Insert into Registation table
	$insertqry="INSERT INTO  `pantra`.`contact` (`c_id` ,`c_patient` ,`c_state` ,`c_district` ,`c_visited_places`,`c_explain_the_activities` ,`c_explanation_activities_date`,`c_explanation_activities_time`,`c_in_date`,`c_up_date`)
VALUES (NULL, '$c_patient', '$c_state', '$c_district', '$c_visited_places', '$c_explain_the_activities','$c_explanation_activities_date','$c_explanation_activities_time','$c_in_date','$c_up_date')";
	$insertresult=mysqli_query($dbconnection,$insertqry);
	//Check whether the query was successful or not
	if($insertresult) 
	{    
        ...
        $msg_type = "Success";
		echo "<script>location='contactView.php?msg=" . urlencode($msg) . "&msg_type=" . urlencode($msg_type) . "'</script>";
        exit();
	} 
	else 
	{
...
if(isset($_POST["btnprimarycontactAdd"]))
{
	...
	//Insert into Registation table
	$insertqry="INSERT INTO  `pantra`.`primary_contact` (`c_p_id` ,`c_p_patient` ,`c_p_person`,`c_p_state`,`c_p_district`,`c_p_town` ,`c_p_place` ,`c_p_in_date`,`c_p_up_date`)
VALUES (NULL, '$c_p_patient','$c_p_person','$c_p_state', '$c_p_district', '$c_p_town', '$c_p_place','$c_p_in_date','$c_p_up_date')";
	$insertresult=mysqli_query($dbconnection,$insertqry);
	//Check whether the query was successful or not
	if($insertresult) 
	{
		$msg      = "Primary Contact successfully added.";
        $msg_type = "Success";
		echo "<script>location='primaryView.php?msg=" . urlencode($msg) . "&msg_type=" . urlencode($msg_type) . "'</script>";
        exit();
	} 
	else 
	{
